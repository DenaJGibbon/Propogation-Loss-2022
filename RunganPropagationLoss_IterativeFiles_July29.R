...
for(b in 1:length(file.name.index)){tryCatch({
  ...
  # Check to make sure number of selections matches the template
    ...
    write.csv(ThirdOctaveBandDFRungan,'ThirdOctaveBandDFRunganAugust1.csv')
    ...
    for(d in 1:nrow(singleplayback.df)){
      ...
      write.csv(BackgroundNoiseRemovedDFRungan,'BackgroundNoiseRemovedDFRunganAugust12022.csv',row.names = F)
    }
...
write.csv(CombinedDF,'BackgroundNoiseRemovedDFRunganAugust12022CombinedDF.csv',row.names = F)
