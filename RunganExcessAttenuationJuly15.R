...
RunganPlot <- ggboxplot(data=observed.prop.lossRunganSubset, y='magic.x',fill='Call.category',
          x='distance',facet.by = 'habitat')#+geom_jitter(width = 1.5, height = 1)

ggboxplot(data=observed.prop.lossRunganSubset, y='dBdoubledist',
                        x='distance',color = 'habitat',facet.by = 'Call.category')#+ylim(0,500)


hist(observed.prop.lossRungan$magic.x)

hist(observed.prop.lossRungan$ExcessAttenuation)
...
# Magic x -----------------------------------------------------------------
playback.line.1 <- median(Hfuntempdf$magic.x)
