...
# Prep Maliau Data
MaliauModelingData <- read.csv('observed.prop.lossMaliauAugust4.csv')
...
observed.prop.lossMaliauSubset <-  observed.prop.lossMaliauSubset%>%mutate(TimeCat = case_when(
  time <= 640  ~ 'Dawn',
...
summary(Combined.lmm.prop.loss.notime)

sjPlot::plot_model(Combined.lmm.prop.loss.notime,intercept=F,sort.est = TRUE)
