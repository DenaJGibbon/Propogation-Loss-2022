...
Maliau.lmm.prop.loss.time.hour <- lmer(magic.x ~ distance + Species*time + (1|date), data=observed.prop.lossMaliauSubset) # + (Call.Type|recorder.ID + Call.Type|recorder.location)
...
MaliauPropLoss <- sjPlot::plot_model(Maliau.lmm.prop.loss.full ,intercept=F,sort.est = TRUE)+ggtitle('Maliau Propogation loss')+ theme_bw()+geom_hline(yintercept = 0)
MaliauPropLoss
