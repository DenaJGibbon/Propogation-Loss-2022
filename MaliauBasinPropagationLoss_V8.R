...
for(b in 1:length(file.name.index.sorted)){ tryCatch({ 
  ...
  for(d in 1:nrow(singleplayback.df)){
    ...
    NoiseWav1 <- cutw(wavfile.temp, from= (Selectiontemp$Begin.Time..s.[1]-timesecs),
                      to=Selectiontemp$Begin.Time..s.[1], output='Wave')
    
    NoiseWav2 <- cutw(wavfile.temp, from= Selectiontemp$Begin.Time..s.[1],
                      to= Selectiontemp$Begin.Time..s.[1]+timesecs, output='Wave')
  ...
  # Take the minimum value from the noise samples
   ...
   w.dn.filt <- bwfilter(SignalWavtemp, 
     ...
     # Normalise the values 
      ...
      write.csv(BackgroundNoiseRemovedDFMaliau,'BackgroundNoiseRemovedMaliauAugust9AdaptiveNoise.csv',row.names = F)
  }
